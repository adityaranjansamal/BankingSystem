...
public class Login {
        ...
        while (true) {// Infinite Loop Until Case 3 exit occurs
            ...
            switch (ch) {
                ...
                case 2:
                    ...
                    while (true) {// while 1
                        System.out.println(
                                "Enter Account Number OR Mail-ID(Format: xyz@abc.com). To Return to Main Menu type Exit");

                        String uid2 = stringsc2.nextLine();
                        if (uid2.equalsIgnoreCase("Exit")) {
                            System.out.println("Returning to Main Menu");
                            Login.whoami();
                            return;
                        } else if (Character.isDigit(uid2.charAt(0))) {// checks if account no. matches with id stored
                                                                       // in
                                                                       // database
                            if (Customer.findAcc(Long.parseLong(uid2))) {
                                Login.pass_checker();
                                break;// while 1 break
                            } else {
                        ...
                        } else if (!Character.isDigit(uid2.charAt(0))) {// checks if mail-id matches with id stored
                                                                        // in
                                                                        // database
                            if (Customer.find_mail_acc(uid2)) {
                                Login.pass_checker();
                                break;// while 1 break
                            } else {
                                System.out.println("Mail-ID Mismatch!");
                                continue;// while 1 continue
                            }
    ...
    private static void pass_checker() {
                ...
                if ((Customer.matchPass(Secrets.hashedpin))
                    ...
                    System.out.println(
                            "Number of Attempts Left:"
                                    + (--Customer.temp.number_of_mpin_attempts));
