...
public class Manager {
    ...
    protected static void createuser() {// Normal LinkedList for customer account creation
        if (head == null) {
            ...
            // New update for Mail ID Insertion is here!
            while (true) {
                System.out.println(
                        "Enter Valid Mail ID. (@gmail.com, @yahoo.com, @outlook.com) are currently supported.");
                n.mail_id = stringsc3.nextLine();
                char mail_character = ' ';
                String check_provider_for_mailbox = "";
                int index_pos = 0;
                for (; index_pos < n.mail_id.length(); index_pos++) {
                    mail_character = n.mail_id.charAt(index_pos);
                    if (mail_character == '@')
                        break;
                }
                // Checks if Mail Provider is Valid or not.
                if (mail_character == '@') {
                    check_provider_for_mailbox = n.mail_id.substring(index_pos);
                    if (check_provider_for_mailbox.equals("@gmail.com")
                            || check_provider_for_mailbox.equals("@yahoo.com")
                            || check_provider_for_mailbox.equals("@outlook.com")) {
                        System.out.println("Mail  Valid! Your Mail for login is:" + n.mail_id);
                        check_provider_for_mailbox = "";
                        index_pos = 0;
                        break;// while loop for mail id checker terminates
                    } else {
                        System.out.println("Mail Provider Not Supported! Enter any supported Mailboxes");
                        index_pos = 0;
                        continue;
                    }
                }
            }
            // End Mail ID Creation
            ...
            while (true) {
                System.out.println(
                        "Enter Valid Mail ID. (@gmail.com, @yahoo.com, @outlook.com) are currently supported.");
                newnode.mail_id = stringsc3.nextLine();
                char mail_character = ' ';
                String check_provider_for_mailbox = "";
                int index_pos = 0;
                for (; index_pos < newnode.mail_id.length(); index_pos++) {
                    mail_character = newnode.mail_id.charAt(index_pos);
                    if (mail_character == '@')
                        break;
                }
                // Checks if Mail Provider is Valid or not.
                if (mail_character == '@') {
                    check_provider_for_mailbox = newnode.mail_id.substring(index_pos);
                    if (check_provider_for_mailbox.equals("@gmail.com")
                            || check_provider_for_mailbox.equals("@yahoo.com")
                            || check_provider_for_mailbox.equals("@outlook.com")) {
                        System.out.println("Mail  Valid! Your Mail for login is:" + newnode.mail_id);
                        check_provider_for_mailbox = "";
                        index_pos = 0;
                        break;// while loop for mail id checker terminates
                    } else {
    ...
    public static void print1(Node k) {
        ...
        System.out.println("Mail-ID:" + k.mail_id);
    ...
    private static void display(Node k) {
        ...
        String str2 = "";
        System.out.printf("%s%8s%s%8s%s%8s%s%8s%s", "NAME", "", "A/CN", "", "BAL", "", "REQ", "", "MAIL");
        while (disp != null) {// iterates over the Linked List
            ...
            System.out.print(disp.request);
            space_counter.setspaces("123");
            System.out.println(disp.mail_id);
            if (!isInSupport)
                disp = disp.next;
            else
                break;
